...
public final class MrimConnection extends ClientConnection {
    ...
    private String getServer() throws JimmException {
        String userDefinedServer = this.mrim.getServer();
        if (userDefinedServer != "") return userDefinedServer;
        // if not defined by user, well, let's use default ones
        byte[] server = new byte[32];
        int size = 0;
        try {
            TcpSocket s = new TcpSocket();
            s.connectForReadingTo("socket://proto.mrim.su:2042");
            s.waitData();
